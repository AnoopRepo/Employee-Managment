...
# ──────────────────────────────────────────────────────────────────────────────
# ─── DOCUMENT ENDPOINTS (ALL USERS - READ PUBLIC, ADMIN - FULL ACCESS) ────────
# ──────────────────────────────────────────────────────────────────────────────

@router.get("/api/documents", response_model=List[DocumentResponse])
async def list_documents(current_user: dict = Depends(get_current_user)):
    db = get_database()
    # Admin gets all, Standard users get only public
    if current_user.get("role") in ("admin", "administrator"):
        cursor = db.documents.find()
    else:
        cursor = db.documents.find({"is_public": True})
        
    documents = []
    async for doc in cursor:
        documents.append(parse_doc(doc))
    return documents

@router.post("/api/admin/documents", response_model=DocumentResponse, status_code=status.HTTP_201_CREATED, dependencies=[Depends(check_admin_role)])
async def create_document(doc_in: DocumentCreate, current_user: dict = Depends(get_current_user)):
    db = get_database()
...
@router.delete("/api/admin/documents/{doc_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(check_admin_role)])
