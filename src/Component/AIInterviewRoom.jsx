...
const AIInterviewRoom = () => {
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-white">
        <div className="relative flex items-center justify-center">
          ...
          <div className="absolute w-12 h-12 border-4 border-blue-500/20 border-t-blue-400 rounded-full animate-spin" style={{ animationDirection: "reverse", animationDuration: "1.5s" }}></div>
        </div>
        <p className="mt-4 text-white/50 tracking-widest text-xs uppercase font-bold animate-pulse">Syncing Interview Handshake...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 text-white text-center">
        <div className="max-w-md p-8 backdrop-blur-xl bg-white/5 border border-red-500/20 rounded-3xl space-y-6 shadow-2xl">
          <div className="text-4xl">⚠️</div>
          <h2 className="text-xl font-bold text-red-400">Handshake Interrupted</h2>
          <p className="text-sm text-white/70 leading-relaxed">{error}</p>
          <Link to="/" className="inline-block py-2.5 px-6 bg-white/10 hover:bg-white/20 border border-white/20 text-xs font-black uppercase tracking-widest rounded-full transition-all">
            Return Home
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col justify-between py-10 px-6 relative overflow-hidden">
      ...
      <div className="absolute bottom-1/4 right-1/4 w-[450px] h-[450px] bg-blue-500/5 rounded-full filter blur-[100px] pointer-events-none"></div>

      {/* Top branding */}
      <header className="max-w-4xl mx-auto w-full flex items-center justify-between z-10">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-lg bg-gradient-to-tr from-purple-500 via-indigo-500 to-blue-600 flex items-center justify-center font-black text-white text-xs">
            W
          </div>
          <span className="font-extrabold text-xs tracking-widest uppercase text-white/80">WorkPulse AI Labs</span>
        </div>
      ...
      <main className="max-w-4xl mx-auto w-full z-10 flex flex-col items-center justify-center flex-1 my-10">
        {!started ? (
          ...
          <div className="max-w-3xl text-center space-y-8 animate-fadeIn w-full">
            <div className="w-20 h-20 bg-gradient-to-tr from-amber-500 via-rose-500 to-indigo-500 rounded-full flex items-center justify-center text-3xl mx-auto shadow-2xl relative group">
            ...
            <div className="space-y-2">
              <p className="text-[10px] font-black uppercase tracking-widest text-purple-400">Interactive Diagnostic Room</p>
              <h2 className="text-2xl md:text-3xl font-black text-white">Hello, {candidate?.name || "Applicant"}</h2>
              <p className="text-white/60 text-xs leading-relaxed max-w-sm mx-auto">
                Welcome to your automated interview for the <strong className="text-purple-300 font-bold">{candidate?.role_applied || "Software Engineer"}</strong> position.
              </p>
            </div>

            {/* Premium AI Agent selector grid */}
            <div className="space-y-4">
              <span className="block text-[10px] font-black uppercase tracking-widest text-indigo-400">Select Conversational AI Recruiter Agent:</span>
                ...
                {Object.keys(agentDetails).map((key) => {
                  ...
                  return (
                    <div
                      ...
                      className={`backdrop-blur-xl bg-slate-900/40 border rounded-2xl p-5 text-center cursor-pointer transition-all duration-300 transform hover:-translate-y-1 ${
                        isSelected 
                          ? `border-purple-500 shadow-lg shadow-purple-500/10 bg-purple-500/[0.04]` 
                          : 'border-white/5 hover:border-white/20'
                      }`}
                    >
                      <div className={`w-12 h-12 rounded-full bg-gradient-to-tr ${agent.color} flex items-center justify-center text-xl mx-auto mb-3 shadow`}>
                        {agent.avatar}
                      </div>
                      <h4 className="font-extrabold text-xs text-white leading-tight">{agent.name}</h4>
                      <p className="text-[8px] font-black uppercase tracking-widest text-purple-400 mt-1">{agent.title}</p>
                      <p className="text-[9px] text-white/40 leading-relaxed mt-2.5 font-medium">{agent.description}</p>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="p-5 bg-white/[0.01] border border-white/5 rounded-2xl space-y-3 text-left text-xs max-w-md mx-auto">
              <span className="text-[9px] font-black uppercase tracking-widest text-indigo-400">Handshake Directives:</span>
              <ul className="space-y-2 text-white/50 text-[11px] leading-relaxed">
            ...
            <button
              ...
              className="px-8 py-3.5 bg-gradient-to-r from-purple-500 to-blue-600 hover:from-purple-600 hover:to-blue-700 text-white font-extrabold text-xs uppercase tracking-widest rounded-full transition-all transform hover:-translate-y-0.5 shadow-xl shadow-purple-500/10 cursor-pointer text-center"
            >
          ...
          <div className="max-w-xl text-center space-y-6 animate-fadeIn">
            <div className="w-16 h-16 bg-emerald-500/10 border border-emerald-500/20 rounded-full flex items-center justify-center text-3xl mx-auto text-emerald-400 shadow-2xl">
              ✓
            </div>
            <div className="space-y-2">
            ...
            {/* AI Fit feedback */}
            <div className="p-6 bg-white/[0.01] border border-white/5 rounded-3xl space-y-4 max-w-md mx-auto shadow-inner text-left">
              <div className="flex justify-between items-center border-b border-white/5 pb-3">
                <span className="text-[9px] font-black uppercase tracking-widest text-indigo-400">Diagnostic Verdict</span>
                <span className="text-xs font-mono font-bold text-emerald-400">SLA Dispatched</span>
              </div>
              <div className="space-y-1.5">
                <span className="text-[9px] font-black uppercase tracking-widest text-white/30">AI Scoring Metrics</span>
                <div className="w-full bg-white/5 border border-white/10 rounded-full h-3 overflow-hidden relative">
                  <div className="bg-gradient-to-r from-purple-500 to-emerald-500 h-full rounded-full transition-all duration-1000" style={{ width: `${gradingResult?.score || 80}%` }}></div>
                </div>
                <div className="flex justify-between text-[10px] font-bold text-white/40">
                  <span>Minimum Grade: 70</span>
                  <span className="text-emerald-400">Achieved Grade: {gradingResult?.score || 80}</span>
                </div>
              </div>
              <p className="text-[11px] text-white/60 leading-relaxed italic bg-white/[0.01] border border-white/5 rounded-xl p-3">
                "{gradingResult?.notes}"
              </p>
            </div>

            <p className="text-[10px] text-white/30 font-semibold tracking-wider uppercase">
          ...
          <div className="w-full space-y-10 animate-fadeIn">
            ...
            <div className="flex justify-between items-center border-b border-white/5 pb-4 w-full">
              <div>
                <span className="text-[9px] font-black uppercase tracking-widest text-purple-400">Dossier Question {currentStep + 1} of {questionsToUse.length}</span>
                <h3 className="font-extrabold text-sm text-white/40 uppercase mt-0.5">{candidate?.role_applied || "Software Developer"}</h3>
              </div>
              <span className={`font-mono text-xs font-bold bg-white/5 border px-2.5 py-1 rounded-xl uppercase tracking-wider ${agentDetails[selectedAgent]?.bgText} ${agentDetails[selectedAgent]?.bgBorder}`}>
                AI Recruiter: {agentDetails[selectedAgent]?.name} Active
              </span>
            </div>

            {/* Glowing friendly recruiter avatar orb */}
            <div className="flex flex-col items-center justify-center gap-4 py-4">
              <div className={`w-24 h-24 rounded-full bg-gradient-to-tr ${agentDetails[selectedAgent]?.color} flex items-center justify-center text-4xl shadow-2xl relative transition-all duration-500 ${recording ? 'scale-110 shadow-rose-500/25' : 'shadow-black'}`}>
                {/* Visual Speech synth pulses */}
                <div className={`absolute inset-0 rounded-full bg-rose-400/20 transition-all ${recording ? 'animate-ping' : 'animate-pulse'}`}></div>
                <div className="absolute inset-2 bg-slate-950 rounded-full flex items-center justify-center">
                  <div className={`w-12 h-12 rounded-full bg-white/[0.03] flex items-center justify-center text-2xl`}>
                    {agentDetails[selectedAgent]?.avatar}
                  </div>
                </div>
              </div>
              <div className="text-center">
                <h4 className="font-extrabold text-xs text-white leading-tight">{agentDetails[selectedAgent]?.name}</h4>
                <p className="text-[8px] font-black uppercase tracking-widest text-purple-400 mt-0.5">{agentDetails[selectedAgent]?.title}</p>
                <p className="text-[9px] text-amber-300 uppercase font-bold tracking-widest animate-pulse mt-2">
            ...
            {/* Question Text card */}
            <div className="p-6 md:p-8 rounded-3xl bg-slate-900 border border-white/5 shadow-2xl text-center space-y-4 relative overflow-hidden">
            ...
            <div className="space-y-6">
              <div className="relative">
                <textarea
                  ...
                  rows="5"
                  disabled={recording}
                  className="w-full px-5 py-4 bg-white/[0.01] hover:bg-white/[0.02] focus:bg-white/[0.02] border border-white/10 rounded-2xl text-xs text-white outline-none focus:border-purple-400 focus:ring-1 focus:ring-purple-400 transition resize-none disabled:opacity-40"
                />
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
                <button
                  ...
                  className={`w-full sm:w-auto px-6 py-3.5 border rounded-xl text-xs font-black uppercase tracking-widest transition-all cursor-pointer flex items-center justify-center gap-2 ${recording ? 'bg-red-500/10 border-red-500/20 text-red-400' : 'bg-white/5 hover:bg-white/10 border-white/10 hover:border-white/20 text-white'}`}
                >
                ...
                <button
                  ...
                  className="w-full sm:w-auto px-8 py-3.5 bg-gradient-to-r from-purple-500 to-blue-600 hover:from-purple-600 hover:to-blue-700 disabled:opacity-30 disabled:pointer-events-none text-white font-extrabold text-xs uppercase tracking-widest rounded-xl shadow-xl transition-all cursor-pointer text-center"
                >
      ...
      {/* Footer copyright */}
      <footer className="max-w-4xl mx-auto w-full text-center z-10 border-t border-white/5 pt-4 text-[9px] text-white/20 font-semibold uppercase tracking-wider">
