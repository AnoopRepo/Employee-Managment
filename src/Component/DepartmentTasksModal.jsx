...
const DepartmentTasksModal = () => {
  ...
  return (
    <>
      ...
      {/* Slide-over drawer */}
      <div 
        className={`fixed top-0 right-0 h-screen w-full max-w-2xl bg-slate-950/95 border-l border-white/10 backdrop-blur-3xl shadow-2xl z-50 flex flex-col transition-transform duration-300 ease-out transform ${
      ...
      >
        ...
        <div className="p-6 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
          <div className="flex items-center gap-3">
            <span className={`text-xs font-black uppercase tracking-widest px-2.5 py-1 rounded border shadow-sm ${
            ...
            <span className="text-white/30 text-xs font-bold">Task Dashboard</span>
          </div>
          <button 
            onClick={closeTask}
            className="w-8 h-8 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center text-white/60 hover:text-white border border-white/5 transition-colors cursor-pointer"
          >
            ✕
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6">
          <h2 className="text-xl md:text-2xl font-black text-white capitalize leading-tight">
            {activeTask.name}
          </h2>
        ...
        {/* Drawer Footer */}
        <div className="p-6 border-t border-white/5 bg-slate-950 flex items-center justify-between text-[10px] text-white/30 font-bold uppercase tracking-wider">
...
const AdminAssetManagement = () => {
  ...
  return (
    <div className="space-y-5">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className={`text-[8px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded ${
                  asset.status === 'In Use' ? 'bg-purple-500/10 text-purple-400 border border-purple-500/20' : 'bg-white/5 text-white/50 border border-white/10'
                }`}>
                  {asset.status}
                </span>
              </div>
              <p className="text-[10px] text-white/40 mt-1 font-mono">SN: {asset.serial} • Type: {asset.type}</p>
            </div>
            <div className="text-right">
              <p className="text-xs text-white/80 font-semibold">Assigned To:</p>
              <p className={`text-xs font-bold ${asset.assignedTo === 'Available' ? 'text-green-400' : 'text-purple-300'}`}>{asset.assignedTo}</p>
      ...
      <form onSubmit={handleAssign} className="p-4 rounded-2xl bg-purple-500/5 border border-purple-500/10 space-y-3">
...
const AdminVendorCoordination = () => {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-4">
        {vendors.map(vendor => (
          <div key={vendor.id} className="p-4 rounded-2xl bg-white/[0.01] border border-white/5 space-y-3">
            <div className="flex justify-between items-start">
              <div>
                <h4 className="text-sm font-bold text-white">{vendor.name}</h4>
                <p className="text-[10px] text-purple-300 font-bold uppercase mt-0.5">{vendor.service}</p>
              </div>
              <button 
                onClick={() => handlePing(vendor.id)}
                disabled={pinging === vendor.id}
                className="px-3 py-1.5 bg-purple-500/10 border border-purple-500/20 hover:bg-purple-500/20 rounded-xl text-purple-300 font-bold text-[10px] uppercase tracking-wider transition-all disabled:opacity-50 cursor-pointer"
              >
            ...
            {vendor.activeLogs.length > 0 && (
              <div className="p-2.5 rounded-xl bg-slate-950 border border-white/5 text-[9px] font-mono text-purple-300/80 max-h-20 overflow-y-auto space-y-1">
          ...
          <div key={rem.id} className={`p-4 rounded-2xl border transition-all duration-300 ${
            ...
            <div className="flex justify-between items-start gap-4">
              <div>
                ...
                <p className="text-xs text-white/50 mt-1 leading-relaxed">{rem.detail}</p>
              </div>
              <div>
                {rem.resolved ? (
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input 
            ...
            className="w-full bg-slate-900 border border-white/15 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-purple-500"
            required
          />
        </div>
        ...
        {status === 'processing' ? (
          <div className="py-2.5 flex items-center justify-center gap-2 bg-purple-500/10 rounded-xl border border-purple-500/20">
            <div className="w-4 h-4 border-2 border-purple-400 border-t-transparent rounded-full animate-spin" />
            <span className="text-xs text-purple-300 font-bold uppercase tracking-wider">Booking Slot & Inviting...</span>
          </div>
        ) : status === 'done' ? (
          <div className="py-2.5 text-center bg-green-500/10 border border-green-500/20 text-green-400 rounded-xl text-xs font-bold uppercase tracking-wider">
...
const AdminExpenseTracking = ({ closeTask }) => {
  ...
  return (
    <div className="space-y-5">
      ...
      <form onSubmit={handleSubmit} className="p-4 rounded-2xl bg-white/[0.01] border border-white/5 space-y-3">
        ...
        <button 
          type="submit"
          className="w-full py-2 bg-purple-500/20 hover:bg-purple-500/30 text-purple-300 rounded-xl text-xs font-bold uppercase tracking-wider transition-colors cursor-pointer border border-purple-500/20"
        >
          File Voucher
        </button>
      </form>
      ...
      <div className="space-y-2">
        <h4 className="text-xs font-bold text-white/50 uppercase tracking-wider">Pending Approvals Ledger</h4>
        <div className="space-y-2">
          {ledger.map((item, idx) => (
            <div key={idx} className="p-3 rounded-xl bg-slate-950 border border-white/5 flex items-center justify-between text-xs font-mono">
              <div>
                <p className="text-white/80 font-bold">{item.desc}</p>
                <p className="text-[10px] text-white/30">{item.date}</p>
              </div>
              <div className="text-right">
                <p className="text-white font-extrabold">${item.amount.toFixed(2)}</p>
                <span className={`text-[8px] font-bold uppercase tracking-wider ${item.status === 'Approved' ? 'text-green-400' : 'text-amber-400'}`}>{item.status}</span>
              </div>
            </div>
          ))}
        </div>
...
const AdminDocumentOrganization = () => {
  ...
  return (
    <div className="space-y-4">
        ...
        {Object.keys(docs).map(folder => (
          <div key={folder} className="space-y-2">
            <button
              ...
              className="w-full p-3.5 rounded-2xl bg-white/[0.01] hover:bg-white/[0.03] border border-white/5 flex items-center justify-between font-bold text-xs text-white text-left transition-colors cursor-pointer"
            >
              <span className="flex items-center gap-2">
                📁 {folder} Files Directory
              </span>
              <span className="text-[10px] text-white/30 font-semibold">{docs[folder].length} assets</span>
            </button>
      ...
      {selectedFile && (
        <div className="p-4 rounded-2xl bg-purple-500/5 border border-purple-500/10 space-y-2 animate-fadeIn">
          ...
          <p className="text-xs text-white/80 font-semibold">{selectedFile.name}</p>
          <p className="text-xs text-white/50 leading-relaxed font-medium">{selectedFile.summary}</p>
        </div>
      )}
    </div>
  );
        ...
        {accounts.map(acc => (
          <div key={acc.id} className="p-4 rounded-2xl bg-white/[0.01] border border-white/5 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded ${
                  acc.status === 'Active' ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'
                }`}>
                  {acc.status}
                </span>
                ...
                <button
                  ...
                  className={`w-9 h-5 rounded-full p-0.5 transition-colors cursor-pointer relative ${
                    acc.status === 'Active' ? 'bg-blue-500' : 'bg-slate-800'
                  }`}
                >
            ...
            <div className="flex justify-between items-center border-t border-white/5 pt-2">
              <span className="text-[9px] text-white/30 font-bold uppercase">{acc.role}</span>
              <button 
    ...
    <div className="space-y-4">
      ...
      <div className="p-4 rounded-2xl bg-slate-950 border border-white/5 space-y-3">
        ...
        <canvas 
          ...
          width="400" 
          height="120" 
          className="w-full bg-slate-900 border border-white/5 rounded-xl"
        />
      </div>
    </div>
  );
...
const ITTicketHandling = ({ closeTask }) => {
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Unassigned', count: 3, color: 'text-red-400 bg-red-500/10 border-red-500/20' },
          { label: 'Pending Dev', count: 2, color: 'text-amber-400 bg-amber-500/10 border-amber-500/20' },
          { label: 'Total Solved', count: 18, color: 'text-green-400 bg-green-500/10 border-green-500/20' }
        ].map(card => (
          <div key={card.label} className={`p-3 rounded-2xl border text-center ${card.color}`}>
      ...
      <div className="p-4 bg-slate-950 border border-white/5 rounded-2xl space-y-2.5">
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={t.id} className="p-2.5 rounded-xl bg-white/[0.01] border border-white/5 flex items-center justify-between text-xs font-mono">
              <div>
                <p className="text-white font-bold">{t.title} <span className="text-blue-400 font-extrabold">{t.id}</span></p>
                <p className="text-[9px] text-white/30">Raised by: {t.author}</p>
              </div>
            ...
            <div key={lic.id} className="p-4 rounded-2xl bg-white/[0.01] border border-white/5 space-y-3">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-[10px] text-white/30 font-mono">Subscription Cost: {lic.cost}</p>
                </div>
                <button
    ...
    <div className="space-y-5">
      ...
      {running ? (
        <div className="p-5 rounded-2xl bg-blue-500/5 border border-blue-500/15 space-y-3">
          <div className="flex justify-between items-center text-xs font-bold text-blue-300 uppercase tracking-wider">
        ...
        <button 
          ...
          className="w-full py-2.5 bg-gradient-to-r from-blue-500 to-indigo-600 rounded-xl text-xs font-bold uppercase text-white hover:opacity-90 active:scale-[0.98] transition-all cursor-pointer shadow-lg shadow-blue-500/10"
        >
      ...
      <div className="space-y-2">
        <h4 className="text-xs font-bold text-white/50 uppercase tracking-wider">Recent Snapshot Registry</h4>
        <div className="space-y-2">
          {backups.map((bak, idx) => (
            <div key={idx} className="p-3 rounded-xl bg-slate-950 border border-white/5 flex items-center justify-between text-xs font-mono">
              <div>
                <p className="text-white/80 font-bold">{bak.name}</p>
                <p className="text-[9px] text-white/30">{bak.date}</p>
              </div>
              <div className="text-right">
                <p className="text-blue-400 font-extrabold">{bak.size}</p>
                <span className="text-[8px] font-bold text-green-400 uppercase tracking-wider">✓ {bak.status}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="p-4 bg-slate-950 border border-white/5 rounded-2xl space-y-4">
        <div className="flex justify-between items-center text-xs">
          <div>
            ...
            <p className="text-[10px] text-white/30 font-mono">Provider: {provider}</p>
          </div>
        ...
        <div className="space-y-1">
          ...
          />
          <div className="flex justify-between text-[8px] text-white/30 font-mono uppercase tracking-widest px-1">
            <span>Scale Min</span>
            <span>Scale Max</span>
          </div>
        </div>
...
const ITHealthAlerts = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="p-4 rounded-2xl bg-slate-950 border border-white/5 space-y-3 font-mono">
        <h4 className="text-[10px] text-white/40 uppercase tracking-widest font-bold">Telemetry Live Log Feed</h4>
        <div className="space-y-2 max-h-48 overflow-y-auto">
          {logs.map(log => (
            <div key={log.id} className="text-[10px] leading-relaxed flex items-start gap-2 border-b border-white/[0.02] pb-1.5">
...
const HRResumeScreening = ({ closeTask }) => {
  return (
    <div className="space-y-4 text-center">
      ...
      <div className="p-5 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 space-y-3 text-left">
        <h4 className="text-xs font-bold text-emerald-400 uppercase tracking-wider">Candidate Screen Overview</h4>
        <div className="space-y-1.5 text-xs text-white/70">
          <p>• Total Applications Loaded: <span className="text-white font-bold">14</span></p>
          <p>• Shortlisted for Interview: <span className="text-white font-bold">5</span></p>
          <p>• AI Screen Completed: <span className="text-white font-bold">2</span></p>
        </div>
      </div>
...
const HRInterviewScheduling = ({ closeTask }) => {
  return (
    <div className="space-y-4 text-center">
      ...
      <div className="p-5 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 space-y-3 text-left">
        <h4 className="text-xs font-bold text-emerald-400 uppercase tracking-wider">Active Recruiter Calendars</h4>
        <div className="space-y-1.5 text-xs text-white/70">
          <p>• Open Time Slots Today: <span className="text-white font-bold">4 slots</span></p>
          <p>• Simulated Invites Dispatched: <span className="text-white font-bold">3 emails</span></p>
        </div>
      </div>
      ...
      <div className="p-4 bg-slate-950 border border-white/5 rounded-2xl space-y-3">
        <div className="flex justify-between items-center text-xs">
          <div>
            ...
            <p className="text-[10px] text-white/30">Target hire: Ellen Ripley</p>
          </div>
      ...
      <div className="space-y-2">
        {workflow.map(w => (
          <button 
            ...
            className="w-full p-3 rounded-xl bg-white/[0.01] hover:bg-white/[0.02] border border-white/5 flex items-center gap-3 text-left text-xs text-white/80 cursor-pointer"
          >
            <div className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 ${
              w.done ? 'bg-emerald-500/20 border-emerald-500/40 text-emerald-400' : 'border-white/20'
            }`}>
              {w.done && '✓'}
            </div>
            <span className={w.done ? 'line-through text-white/30' : ''}>{w.item}</span>
          </button>
        ))}
      </div>
    </div>
...
const HRAttendanceTracking = ({ closeTask }) => {
  return (
    <div className="space-y-4 text-center">
      ...
      <div className="p-5 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 space-y-3 text-left">
        <h4 className="text-xs font-bold text-emerald-400 uppercase tracking-wider">Weekly Attendance Stats</h4>
        <div className="space-y-1.5 text-xs text-white/70">
          <p>• Today's On-time Clock-ins: <span className="text-white font-bold">96%</span></p>
          <p>• Missing Check-ins: <span className="text-white font-bold">1 team member</span></p>
        </div>
      </div>
...
const HRLeaveManagement = ({ closeTask }) => {
  return (
    <div className="space-y-4 text-center">
      ...
      <div className="p-5 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 space-y-3 text-left">
        <h4 className="text-xs font-bold text-emerald-400 uppercase tracking-wider">Pending Leave requests</h4>
        <div className="space-y-1.5 text-xs text-white/70 font-medium">
          <p>• Vacation (John Doe): <span className="text-amber-400 font-bold">3 days pending</span></p>
          <p>• Personal (Sarah Connor): <span className="text-green-400 font-bold">1 day approved</span></p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {folders.map(fold => (
          <div key={fold.name} className="p-4 rounded-2xl bg-white/[0.01] hover:bg-white/[0.02] border border-white/5 text-left space-y-2 cursor-pointer transition-colors">
            ...
            <h4 className="text-xs font-bold text-white">{fold.name}</h4>
            <p className="text-[10px] text-white/30 font-semibold">{fold.files} signed records</p>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3.5">
        {policies.map(pol => (
          <div key={pol.name} className="p-4 rounded-2xl bg-white/[0.01] border border-white/5 space-y-2">
            <div className="flex justify-between items-center text-xs">
              <span className="font-bold text-white/80">{pol.name}</span>
              <span className="text-emerald-400 font-extrabold">{pol.rate}%</span>
            </div>
            <div className="w-full h-1.5 bg-slate-900 border border-white/5 rounded-full overflow-hidden">
              <div className="h-full bg-emerald-500" style={{ width: `${pol.rate}%` }} />
            </div>
          </div>
        ))}
      </div>
...
const HRTrainingTracking = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3.5">
        {courses.map(course => (
          <div key={course.name} className="p-4 rounded-2xl bg-white/[0.01] border border-white/5 space-y-2">
            <div className="flex justify-between items-center text-xs">
              <span className="font-bold text-white/80">{course.name}</span>
              <span className="text-emerald-400 font-extrabold">{course.rate}%</span>
            </div>
            <div className="w-full h-1.5 bg-slate-900 border border-white/5 rounded-full overflow-hidden">
              <div className="h-full bg-emerald-500" style={{ width: `${course.rate}%` }} />
            </div>
          </div>
        ))}
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-4">
        ...
        <div>
          ...
          <textarea 
            ...
            className="w-full bg-slate-900 border border-white/15 rounded-xl p-3 text-xs text-white focus:outline-none focus:border-emerald-500 resize-none"
          />
        </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2.5">
        {metrics.map(met => (
          <div key={met.label} className="p-2.5 rounded-xl bg-white/[0.01] border border-white/5 text-center">
      ...
      <form onSubmit={handleSubmit} className="p-3 rounded-2xl bg-white/[0.01] border border-white/5 space-y-2">
        <input 
          type="text"
          value={newFeedback}
        ...
        <button 
          type="submit"
          className="w-full py-1.5 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-colors cursor-pointer"
        >
          Submit Anonymously
        </button>
      </form>
      ...
      <div className="space-y-1.5 max-h-24 overflow-y-auto">
        {feedbacks.map((f, idx) => (
          <p key={idx} className="p-2 rounded-lg bg-slate-950 border border-white/[0.02] text-[10px] text-white/60 leading-relaxed font-medium">
            • "{f}"
          </p>
        ))}
      </div>
...
const FallbackTask = ({ taskName }) => {
  return (
    <div className="text-center py-6 space-y-2">
      ...
      <h3 className="text-white font-bold text-sm">Dashboard Active</h3>
